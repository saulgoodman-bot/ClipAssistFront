...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
       ...
       // Assuming backend returns a token and user
       login(res.access_token, { user_id: res.user_id, email: res.email, plan: res.plan });
       // We can simply navigate since we don't have Next.js api routes for cookie setting
       navigate('/upload');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl relative">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <p className="text-sm text-zinc-400 mt-2">Sign in to your ClipAssist account</p>
          <p className="text-xs text-indigo-400 mt-4 bg-indigo-500/10 p-2 rounded border border-indigo-500/20">
            For demo purposes, entering any credentials will log you in without a backend.
          </p>
        </div>
